...
class BaseMinimizer(ABC):
    def __init__(
        ...
        self._dont_cares = self._get_excess_value()
    ...
    @property
    def dont_cares(self) -> list[str]:
        return self._dont_cares
    ...
    def _get_excess_value(self) -> list[str]:
        return [
            "".join(str(value) for value in row[:-1])
            for row in self.truth_table
            if row[-1] == self._excess_value
        ]
    ...
    def get_prime_implicants(self) -> list[str]:
        current_terms = set(self.terms) | set(self.dont_cares)
